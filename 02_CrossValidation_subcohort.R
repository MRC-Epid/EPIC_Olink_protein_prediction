#########################################
####  02_CrossValidation_subcohort   ####
#########################################

options(stringsAsFactors = F)

## This script runs validation of predictive proteins by cross-validation   
...
lasso <- lapply(1:nrow(d.labels.20), function(x){
  ...
  ## Read in top 20 proteins from feature selection  
  fs.20 <- read.delim(paste0("../data_output/final_features_top20/",d.labels.20$index_event[x],"_set1.txt"), sep = "\t")
  
  ## restrict to top 5 predictors according to selection score
  predictors <- fs.20 %>%
  ...
  clinical.protein.coxnet <- coxnet.cv(Train.data = dat,
                                       ...
                                       Train.surv.data = surv.dat,
                                       times =100 )
  clinical.grs.coxnet <- coxnet.cv(Train.data = dat,
                                   predictors =c(grs.list[[d.labels.20$index_event[x]]], clin.vars) ,
                                   Train.surv.data = surv.dat,
                                   times =100)
  clinical.protein.grs.coxnet <- coxnet.cv(Train.data = dat,
                                           predictors =c(predictors, clin.vars,grs.list[[d.labels.20$index_event[x]]]) ,
                                           Train.surv.data = surv.dat,
                                           times =100 )
  ...
  res.list <- list(protein.model=protein.coxnet,
                   ...
                   clinical.grs.coxnet=clinical.grs.coxnet,
                   clinical.protein.grs.coxnet = clinical.protein.grs.coxnet,
                   n=nrow(dat),
...
cindex <- t(sapply(d.labels.20$index_event, function(x){
  return(data.frame( protein.model= lasso[[x]]$protein.model$Cindex.mean,
                     ...
                     clinical.protein.grs.coxnet= lasso[[x]]$clinical.protein.grs.coxnet$Cindex.mean,
                     clinical.grs.model =lasso[[x]]$clinical.grs.coxnet$Cindex.mean))
}))
...
low.ci <- t(sapply(d.labels.20$index_event, function(x){
  return(data.frame( protein.model= lasso[[x]]$protein.model$ci.low,
                     ...
                     clinical.protein.grs.coxnet= lasso[[x]]$clinical.protein.grs.coxnet$ci.low,
                     clinical.grs.model =lasso[[x]]$clinical.grs.coxnet$ci.low))
}))
...
up.ci <- t(sapply(d.labels.20$index_event, function(x){
  return(data.frame( protein.model= lasso[[x]]$protein.model$ci.upper,
                     ...
                     clinical.protein.grs.coxnet= lasso[[x]]$clinical.protein.grs.coxnet$ci.upper,
                     clinical.grs.model =lasso[[x]]$clinical.grs.coxnet$ci.upper))
}))
up.ci <- as.data.frame(up.ci)
up.ci$disease <- row.names(up.ci)

## long format to merge
cindex <- gather(cindex, model, mean.Cindex, 1:5)
low.ci <- gather(low.ci, model , low.ci, 1:5)
up.ci <- gather(up.ci, model , up.ci, 1:5)
...
write.table(ci.res, file = "../data_output/cindex_subcohort_results.txt", sep = "\t", row.names = F)
